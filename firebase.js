...
class FirebaseAuth {
  constructor() {
    this.firebaseConfig = {
      apiKey: "AIzaSyAYtzK1L9_Cksm6zfDtc-B5tiRwxiCTcds",
      authDomain: "rick-and-morty-ee111.firebaseapp.com",
      projectId: "rick-and-morty-ee111",
      storageBucket: "rick-and-morty-ee111.appspot.com",
      messagingSenderId: "829355727299",
      appId: "1:829355727299:web:5aa65c5dcabf76cac44576",
    };

    this.appInstance = initializeApp(this.firebaseConfig);
    this.appAuth = getAuth(this.appInstance);
  }

  async handleLogin({ auth, email, password, onError }) {
    try {
      await signInWithEmailAndPassword(auth, email, password);
    } catch (error) {
      onError(this.verifyErrorCode(error.code));
    }
  }

  async handleCreateAccount({ auth, email, password, onError }) {
    try {
  ...
  verifyErrorCode(errorCode) {
    // doc: https://firebase.google.com/docs/reference/js/firebase.auth.Auth
    // doc: https://firebase.google.com/docs/auth/admin/errors?hl=pt-br
    switch (errorCode) {
